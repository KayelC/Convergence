...
public sealed class CatalogLoaderTests
{
    ...
    public void CatalogQualification_CoversContentReferencesButLeavesHostVocabularyLocal()
    {
        ...
        Assert.Equal(PassiveTriggerTargetScope.Owner, trigger.Targeting.Scope);
        Assert.Equal(TargetLifeState.Any, trigger.Targeting.LifeState);
        Assert.True(trigger.Targeting.IncludeReserveActors);
        Assert.Equal(Id("companion"), entity.EntityKindId);
        Assert.Contains(Id("strength"), entity.Stats.Keys);
    }
    ...
    public void ParsingAndValidationFailures_AreSerializerNeutralAndPreventCatalogCreation()
    {
        ...
        ContentPackTextBundle validationFailure = Bundle(
            ...
            Document("skills.json",
                "{\"schemaVersion\":10,\"skills\":[{\"id\":\"empty_passive\",\"displayName\":\"Empty\",\"description\":\"Empty.\",\"activation\":\"passive\",\"inheritanceGroupId\":\"passive\",\"inheritance\":{\"isInheritable\":true}}]}"));
    ...
    private static string RaceDocument(string id) => $$"""
        { "schemaVersion": 10, "races": [{ "id": "{{id}}", "displayName": "{{id}}" }] }
        """;
    ...
    private static string AilmentDocument(string id) => $$"""
        {
          ...
          "ailments": [{
            ...
            "defaultLifetime": {
              "expiration": { "type": "turns", "value": 3, "tick": "owner_turn_end", "suspendWhileReserve": false },
              "allowedRemovalCauses": ["duration_expired", "battle_end"]
            },
