...
public sealed class SkillSystemRedesignFixtureTests
{
    ...
    public void SamplePack_EncodesPassiveFusionFodderInvariant()
    {
        ...
        Assert.Equal(4, RequireInt(manifest, "schemaVersion"));
        Assert.Equal(4, RequireInt(skillDocument, "schemaVersion"));
        Assert.Equal(4, RequireInt(entityDocument, "schemaVersion"));
        Assert.Equal(4, RequireInt(raceDocument, "schemaVersion"));
