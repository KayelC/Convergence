...
public sealed class SkillSystemRedesignFixtureTests
{
    ...
    public void SamplePack_EncodesPassiveFusionFodderInvariant()
    {
        ...
        Assert.Equal(7, RequireInt(manifest, "schemaVersion"));
        Assert.Equal(7, RequireInt(skillDocument, "schemaVersion"));
        Assert.Equal(7, RequireInt(entityDocument, "schemaVersion"));
        Assert.Equal(7, RequireInt(raceDocument, "schemaVersion"));
