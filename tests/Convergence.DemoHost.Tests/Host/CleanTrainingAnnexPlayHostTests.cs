...
public sealed class CleanTrainingAnnexPlayHostTests
{
    ...
    public async Task CleanTrainingAnnexPlay_ShopBuysCatalogItemThroughBoundTransactions()
    {
        ...
        Assert.Equal(
            ...
            summary.Commands);
        Assert.Contains(
            "Shop transaction: Bought Annex Tonic for 47 M; wallet 100->53; quantity 1->2.",
            output.ToString(),
            StringComparison.Ordinal);
        io.AssertConsumed();
    }
    ...
    public async Task CleanTrainingAnnexPlay_ShopBuysAndEquipsCatalogEquipment()
    {
        ...
        Assert.Equal(
            [
                "Annex Tonic - 47 M",
                "Cleanse Drop - 37 M (stock 5)",
                "Practice Blade - 112 M [Already owned]",
                "Padded Jacket - 84 M",
                "Back"
            ],
            buyMenu.Options);
        ...
        string text = output.ToString();
        Assert.Contains(
            "Shop transaction: Bought Padded Jacket for 84 M; wallet 100->16; owned 0->1.",
            text,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    public async Task CleanTrainingAnnexPlay_ShopInsufficientFundsAreDisabledWithoutMutation()
    {
        ...
        Assert.Equal(
            [
                "Annex Tonic - 47 M [Not enough Macca]",
                "Cleanse Drop - 37 M (stock 5) [Not enough Macca]",
                "Practice Blade - 112 M [Already owned]",
                "Padded Jacket - 84 M [Not enough Macca]",
                "Back"
            ],
            buyMenu.Options);
    ...
    public async Task CleanTrainingAnnexPlay_ShopSellsCatalogItemAndBlocksEquippedSale()
    {
        ...
        Assert.Equal(
            [
                "Annex Tonic - 28 M (owned 1)",
                "Practice Blade - 67 M (owned) [Equipped]",
                "Back"
            ],
            sellMenu.Options);
        Assert.Equal([false, true, false], sellMenu.DisabledOptions);
        Assert.Contains(
            "Shop transaction: Sold Annex Tonic for 28 M; wallet 0->28; quantity 1->0.",
            output.ToString(),
            StringComparison.Ordinal);
        io.AssertConsumed();
    }
