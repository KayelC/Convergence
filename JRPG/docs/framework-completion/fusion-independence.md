...
Phase 7-35 completed the separate clean Compendium proof without expanding the recipe chart. CodeReview-7-1 preserves typed recipes, CodeReview-7-2 improves recall identity, CodeReview-7-3 centralizes commit coordination, CodeReview-7-4 retains strategy context, and CodeReview-7-5 hardens entry/save validation. The fresh 2026-07-11 audit reopened Phase 7 for four medium findings; parent-order-neutral rank state, framework-owned preview authority, Persona stock-capacity validation, and duplicate-knowledge rejection are all corrected. Subsequent low-priority follow-ups apply the global identity rule to direct party additions and make Compendium recall pricing an optional host policy with free and registration-only configurations. Phase 7 is review-closed for Phase 8 with no source finding deferred. Keep future fusion policies concept-first and explicitly detached from ATLUS race charts, demon names, spell families, and named currencies.
