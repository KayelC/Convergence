...
Phase 7-30 adds a clean catalog fusion repository for original content. `CatalogFusionContentRepository` reads qualified `GameDataCatalog` fusion recipes, entities, and skills into the same framework fusion runtime contracts used by the legacy adapter. The Training Annex clean host uses that path only for non-mutating result calculation; it does not replace Cathedral rituals, inheritance selection, transactions, accidents, or Compendium presentation.
