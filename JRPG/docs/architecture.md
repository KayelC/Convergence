...
Phase 6-29 adds a clean negotiation/recruitment proof on original content. Training Annex now presents `Negotiate / Recruit`, maps answers and demand choices through host command contracts, runs `NegotiationSessionService`, validates recruitment through `RecruitmentTransactionService`, adds the recruited Bramble Runner through `PartyStockTransitionService.AddDemonToStock`, and spends Macca through the bound economy service. The framework owns the session outcome and mutation rules; the console host owns prompt presentation and the session-local familiar/duplicate guard. Authored demand definitions are present in content, but the current framework service still calculates the Macca demand internally, so demand-policy binding remains future work.
...
Phase 7-30 adds a clean catalog fusion repository for original content. `CatalogFusionContentRepository` reads qualified `GameDataCatalog` fusion recipes, entities, and skills into the same framework fusion runtime contracts used by the legacy adapter. Phase 7-31 routes that catalog path through `FusionPlanningService` as well, proving inheritance slot calculation, passive/explicit-allowance filtering, display reason codes, and deterministic accident mutation evidence. Phase 7-32 adds a small Training Annex preview command: inherited-skill choices are host-presented, validated through the framework inheritance validator, and turned into a `FusionPreviewSnapshot` without mutating runtime state. Phase 7-33 adds a Training Annex transaction proof: duplicate results are rejected before mutation, while a valid commit consumes parent demons and adds a hydrated fused result through framework party/stock transitions. Phase 7-34 makes the strategy layer explicit: `FusionPolicyRegistry` is required by resolution/planning, policy context is optional host data, authored policy IDs are preserved from catalog recipes, and no default policy set is hidden inside the framework. Neutral create/rank operations remain built in; stat boost, special results, accidents, mutation, sacrifice, and slot behavior are selected by registered policies. Legacy Moon Phase and catalyst assumptions are implemented only by `LegacyFusionStrategyPolicies` in the console host. CodeReview-7-1 makes schema v1 explicitly binary and preserves typed entity/race parent selectors through catalog mapping and runtime resolution; structured results no longer require legacy compatibility strings. The Training Annex clean host still does not replace Cathedral rituals or Compendium presentation.
