...
CodeReview-1 advances the pre-release save contract to version `3`. Actor snapshots now include the vital-resource ID and exact typed durations, and catalog restore reconstructs complete canonical actors without invoking creation defaults. This remains a pre-release contract; no production save migration is required yet.
