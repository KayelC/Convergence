...
The M2 correction defines a shared numeric contract for that boundary. Base and effective stat values are nonnegative decimals no greater than `Int32.MaxValue`; fractional values remain valid and are resolved by the selected stat policy. Base-resource values are nonnegative decimals with no framework balance ceiling. Snapshot objects remain capable of carrying untrusted host data so validation can aggregate every fault, but `RuntimeSaveValidator`, direct actor restore, direct actor construction, and progression replacement all enforce the same `RuntimeActorNumericDomain` before state is published. Standard stat, resource-growth, and natural-recovery arithmetic caps or saturates accepted boundary values instead of leaking conversion or decimal-overflow exceptions.
...
The M1 authority correction makes that shared path literal. `BattleActionAssessment`, `SkillExecutionAssessment`, and `ItemExecutionAssessment` are immutable, executor-owned, single-use preparation tokens. They retain ordered target IDs rather than live target collections, execution rebinds those IDs into the staged actor transaction, and random-target policies are never called a second time. The Training Annex field/battle adapters and automated selector execute the exact assessment they presented or selected.

The L2 result-boundary correction applies the same ownership rule after effect execution. `EffectExecutionResult` snapshots its outer passive-activation and host-action-request collections both during construction and whenever a custom handler replaces them through a record `with` expression. The public `init` surface remains available to host extensions, but neither outer property can retain a mutable caller-owned collection alias.
...
The H1 lifecycle correction adds `BattleDurationLifecycleService` as the shared duration boundary beneath that status service. The outermost ordered effect action expires instant state after nested trigger execution; encounter phase callbacks expire matching phase state; and battle/field cleanup removes encounter-scoped state without deleting permanent state. All duration-bearing actor stores use the same policy, including generic other statuses, and the automated clean runner and Training Annex lifecycle adapter consume the phase and battle boundaries.

The M3 authority correction removes the automated runner's second, reduced rule path. `BattleStatusEncounterLifecyclePort` now adapts the full status/passive lifecycle to encounter callbacks, while `AutomatedBattleRunner` requires an explicit lifecycle port and `BattleTurnEconomyRuleset`. The runner cannot construct Press Turn, choose phase liveness limits, or substitute a no-op lifecycle internally. A host may still choose another lifecycle or turn economy because both systems are optional policies, but that choice is visible at its composition root and is shared with direct `BattleEncounterRunner` use.

The L1 encounter-boundary correction requires every participant in one encounter request to have a unique `RuntimeInstanceId`. `BattleEncounterRunner` rejects all duplicate groups before initiative, synchronization, lifecycle, turn-economy construction, or command execution. The fault is returned and published with the stable `DuplicateParticipantInstanceId` code, and `AutomatedBattleRunner` preserves that code in its projected result and event stream.
...
The L3 pricing-boundary correction makes public shop arithmetic representation-safe. Runtime offers reject negative base prices at construction and record-clone assignment, Luck must be nonnegative, and one checked decimal calculator owns both direct calculation and transaction assessment. An unrepresentable price throws from the direct calculator or returns the typed non-mutating `InvalidShopPricing` transaction result; it is never wrapped, clamped, or published as a successful sentinel price.
...
Restore validation reports base-stat, effective-stat, and base-resource violations with separate stable codes and precise actor paths. `RequireValidSnapshot()` therefore guarantees that these numeric domains are safe for ordinary framework policy consumption; it does not silently clamp corrupt persisted values. Clamping and saturation are reserved for arithmetic on values already accepted by the contract.
