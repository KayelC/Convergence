...
Phase 2-18 adds session-owned clean battle knowledge to that path. The Training Annex host keeps framework elemental, ailment, and instant-death knowledge stores for the session, learns from typed action results and Analyze, feeds elemental discoveries back into the framework selector, and places the resulting `RuntimeKnowledgeSnapshot` into the save-facing summary/validation path. The host still owns presentation; knowledge storage and validation use framework snapshot contracts. This pass also exposed an ownership correction that must be made before reward completion: persistent player discoveries and per-encounter AI discoveries are separate scopes. Saves and future UI hints use player knowledge; random-encounter AI starts with fresh encounter knowledge and discards it after battle.
