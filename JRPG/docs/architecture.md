...
Phase 5-26 adds active/reserve party ownership to the same original clean host without introducing Persona/Demon stock yet. Training Annex hydrates Echo Adept as the active actor and Annex Mentor as a reserve support actor, then builds the live `RuntimePartyStockSnapshot` through `PartyStockTransitionService.AddPartyMember`. The host owns `Inspect Party` presentation; the framework owns the party snapshot and transition result. Manual and suspend saves now persist and restore that live party stock with the actor roster.
