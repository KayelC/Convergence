...
Track Q1 adds the production-content audit ledger before any real content conversion. Track Q2 amends the direction: the legacy `Data/Jsons` records are prototype-only and not approved as commercial/shippable clean content. The ledger still accounts for skills, entities, races, ailments, items, equipment, shops, negotiations, encounters, dungeons, fusion recipes, and rulesets, and it keeps old v2 migration outputs as historical evidence only. Track T3 adds `convergence.training_annex_slice`, the first original clean content seed, with one race, two entities, three skills, one item, one encounter, one dungeon, and eight rulesets. Track T4 adds `--clean-training-annex-demo`, which loads only that pack and proves a complete clean noninteractive loop: ruleset binding, actor hydration, dungeon traversal, item use, automated battle, rewards, progression, and save validation. Production gameplay still reads the legacy `Data/Jsons` files through `Database` until broader original clean consumers are explicitly switched.
