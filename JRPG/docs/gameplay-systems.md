...
Phase 2-18 gives the Training Annex clean battle a real session knowledge state. Damage effects learn elemental affinity from resolved typed effect results, Analyze learns elemental/ailment/instant-death defenses from the target's typed profile, and the learned elemental store is reused by framework enemy selection. The save-facing summary carries the resulting `RuntimeKnowledgeSnapshot`, and validation checks it against the catalog. Before Phase 2-19, that single store must be split into persistent player knowledge and per-encounter AI knowledge: the player keeps discovered target hints across battles, while ordinary enemies learn only within the current battle unless a host intentionally supplies special persistent knowledge.
