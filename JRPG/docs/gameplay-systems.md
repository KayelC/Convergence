...
The save-file format remains host-owned. `--clean-save-demo` proves a console host can serialize the framework snapshot through `System.Text.Json`, deserialize it, validate it, rebuild framework runtime actor state, and exit without input.
...
Restored Training Annex state includes actor runtime snapshots, HP/SP synchronization back into clean battle actors, inventory, wallet, generic navigation/dungeon state, session counters/flags, and persistent player battle knowledge. Permanent save files, autosaves, battle saves, Godot save resources, save-version migrations, and legacy prototype save/load remain outside this phase.
