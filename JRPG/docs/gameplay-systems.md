...
Phase 7-30 adds the first original clean fusion result proof. `CatalogFusionContentRepository` adapts `GameDataCatalog` fusion/entity/skill definitions directly into the framework fusion runtime. Phase 7-31 extends the same proof into inheritance planning, and Phase 7-32 adds non-mutating preview confirmation. Phase 7-33 adds committed transaction evidence. CodeReview-7-3 moves the transaction algorithm into `FusionTransactionService`: only a validated inheritance token can be prepared, owner kind selects Demon or Persona transitions, injected stock policy decides capacity, and actor construction happens after host confirmation. Preparation fixes participant identities, result ownership, and optional retained stat-boost state; rejected or stale commits preserve the authoritative input snapshot and publish no applied consumption. Phase 7-34 makes strategy rules modular: hosts explicitly register slot, sacrifice, accident, mutation, catalyst/result, and compatibility policies. Fusion can run without Moon Phase metadata, sacrifice can be disabled or progression-gated, and stat boosts use typed catalyst IDs and policy results rather than names. CodeReview-7-4 guarantees that the same immutable host/session context used during planning reaches every accident mutation and provides an explicit contextual slot-count path. Phase 7-35 adds clean Compendium registration/recall as an atomic framework transaction and persists the result through the Training Annex save path. CodeReview-7-1 preserves authored recipe structure and CodeReview-7-2 enforces runtime identity.
...
The Compendium now uses framework state/recall contracts for registration, overwrite, recall pricing, and recall assessment while retaining the existing console registry API. O10 exposes detailed presentation records for registration, recall menu selection, recall assessment, and recall transaction outcomes. Registered entries are deep snapshots, including active Persona state; mutating a recalled clone no longer mutates the stored entry. The legacy console explicitly selects its historical base-price plus level/stat/skill coefficients through `LinearCompendiumRecallPricingPolicy`; those values are no longer framework defaults.

Phase 7-35 adds the clean runtime counterpart. `CompendiumRuntimeService` snapshots a clean actor's qualified entity ID, progression, integral base stats, learned skills, and equipped skills; catalog reconstruction restores those durable values while clearing transient battle/equipment state and refilling recalculated resources. Recall checks duplicate entity ownership, selected Demon/Persona stock capacity, an explicit pricing decision, and available balance before returning any changed snapshot. Registration does not require recall. A host may omit recall pricing, choose free recall, inject another progression-aware policy, and name or present currency independently. Host code applies returned party/wallet state only when the result is `Applied`.

CodeReview-7-2 adds a separate runtime-identity guard before recall can create an actor or charge the wallet. The proposed recalled ID must be absent from owner, active party, reserve party, active form, Persona stock, and Demon stock. Party-member and stock addition/replacement commands enforce the same rule while preserving the exact owner-reference plus active-party representation. Existing duplicates in the destination still report `DuplicateOwned`; cross-role reuse reports the distinct typed identity-collision code. Save validation applies the same ownership model, checks that each reference's entity ID matches its actor snapshot, and enforces the injected owner-level capacity for both Demon and Persona stock. The intended active-party plus Demon-stock representation remains valid through the explicit summon transition.
...
Phase 7-35 implements the optional player-knowledge import boundary. `FamiliarEntityKnowledgeService` accepts explicit familiar entity IDs and imports their typed elemental, ailment, and instant-death defenses from the catalog into a new player knowledge snapshot. Training Annex invokes it after clean recruitment, committed fusion, Compendium registration, and recall. The service is not automatic framework global state, so a developer may omit it or choose a different familiarity policy. Ordinary enemy AI still receives a fresh per-encounter knowledge store. Duplicate keys in any knowledge channel are rejected through the same integrity rule used by save validation; direct import returns typed diagnostics without mutation or dictionary exceptions.
