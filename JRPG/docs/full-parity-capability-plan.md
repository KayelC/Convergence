...
Phase 7 implementation status: passes 7-30 through 7-35 and CodeReview7-1 through CodeReview7-5 are present. A fresh source audit on 2026-07-11 reopened the review gate, and all four medium findings and both low-priority findings were corrected by 2026-07-12. Neutral rank results are parent-order independent, previews require planner-issued validated selections, restored Demon/Persona stock shares live capacity policy, duplicate persisted knowledge is rejected before save restore or familiar import can consume it, direct party additions enforce the same global runtime-ID rule as stock additions without removing intentional overlaps, and Compendium registration no longer forces paid recall or a named currency. Phase 7 is review-closed and Phase 8 may begin. Capabilities remain `parallel_partial` and removal remains unauthorized; see [Phase 7 Fresh Code Review And Readiness](../ArchiveDocs/Reviews/phase-7-code-review.md).
...
- The L5 threading correction makes `RunAsync` the engine-host contract, removes synchronization-context capture from orchestration, and confines synchronous blocking to a context-detaching compatibility helper. Hosts own any UI or engine-thread marshaling; capability status and removal authorization remain unchanged.
...
- party-member additions and Demon/Persona stock additions and replacements reject cross-role runtime-ID reuse with a typed transition code;
...
- the capability remains `parallel_partial`; the 2026-07-11 fresh source audit reopened the Phase 7 gate for the cross-cutting persistence and framework-contract findings recorded in `ArchiveDocs/Reviews/phase-7-code-review.md`.
...
- Runtime-ID follow-up verification: all `23/23` party/stock transition tests and `948/948` full-suite tests passed. The exact owner reference may occupy an open active slot, the intentional active-party plus Demon-stock overlap remains available only through `SummonDemon`, and direct additions reject form/stock reuse without mutation.
- Compendium-pricing follow-up: hosts now explicitly choose no recall, free recall, the configurable linear helper, or another `ICompendiumRecallPricingPolicy`. Legacy Cathedral and Training Annex explicitly preserve their former coefficients; focused policy/host/boundary/legacy tests passed `143/143` and the full suite passed `951/951`.
