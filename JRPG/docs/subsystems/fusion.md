...
Neutral structured `rank_offset` results are parent-order neutral at the state layer as well as the identity layer. They start from the resolved catalog entity's authored stats. A rank policy may preserve parent state only by explicitly returning the transformed parent; argument position is never treated as an implicit target.
...
Phase 7-32 adds `Preview Fusion Result` to the clean Training Annex host. The host presents inherited-skill choices for a sacrificial Echo Adept + Bramble Runner + Ashling sample. `FusionPlanningService` validates those IDs against the exact evaluated inheritance plan retained by `FusionPlanningResult`; `FusionPreviewRequest` accepts only the resulting `ValidatedFusionInheritanceSelection` token. Confirmation records the accepted preview but intentionally does not mutate party/stock, parent actors, inventory, wallet, or Compendium state.
