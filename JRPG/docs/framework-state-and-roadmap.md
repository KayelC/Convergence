...
- Phase 1-07 gives that shell its first generic navigation boundary: arbitrary location IDs, explicit transitions, injected access policy, live navigation inspection, and save validation over optional field state. The console menu is only one host adapter; Godot triggers, VN hotspots, or scripts can request the same transition. Dungeon traversal and encounter triggering remain separate optional follow-up passes.
