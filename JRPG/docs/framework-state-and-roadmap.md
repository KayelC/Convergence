...
- Phase 1-07 gives that shell its first generic outer-navigation boundary: arbitrary location IDs, explicit transitions, injected access policy, live navigation inspection, and save validation over optional field state. Phase 1-08 adds a separate optional dungeon-node boundary with explicit policy-checked transitions, visited nodes, checkpoints, barrier rejection, and boss flags. Console menus are only one host adapter; Godot triggers, VN hotspots, doors, or scripts can request the same operations. Encounter triggering remains the independent Phase 1-09 pass.
