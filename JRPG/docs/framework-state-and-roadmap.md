...
- resource changes, revival, cure/removal, stat stages, charge, shield, affinity Break, affinity override, analysis, escape, and custom host requests.
...
Review-Whole-3 hardens the framework beneath that host proof. Clean skills, items, passives, and direct lifecycle calls share one ailment-application authority. Turn-start restrictions preserve allowed action IDs and combine all active ailments through an injected policy; custom behavior requires a handler instead of silently becoming `CanAct`. Ailment trigger conditions and failure policies use the shared ordered-effect engine, while stat stages and restored snapshots enforce `-4..+4`. This resolves the framework-internal ailment contradictions, but it does not retire protected legacy content or consumers.

Review-Whole-4 hardens the shared party/stock transition authority. Demon-specific swap, return, replace, and consume operations prove the subject is owned in Demon Stock before mutation; deployment operations additionally require active membership. The previous active-only replacement fallback is gone, owner/ordinary-party attacks are regression-tested, and valid active-plus-owned or standby-owned demon flows remain intact. Party/stock capability status remains `parallel_partial` while protected legacy consumers remain in use.

Review-Whole-5 hardens fusion authority without changing content or legacy Cathedral ownership. Equal-specificity recipe overlaps are rejected by content validation and fail closed in runtime repositories, while specific-over-broad matching remains supported. Accident inheritance is now exact-plan-only: the framework owns its pool and limit, registered mutation outputs are revalidated, and previews/transactions accept only the resulting plan-bound token. Verification passed `214/214` focused tests and `984/984` full-suite tests; the framework retained `0` warnings and all four clean demos passed. Fusion capabilities remain `parallel_partial` and legacy removal remains unauthorized.

Review-Whole-6 hardens execution and numeric boundaries. Clean skills, items, and direct effects now stage complete actor state and publish it only after successful ordered execution; item actions also wait for a typed atomic host inventory commit. Registered extension failures become typed diagnostics instead of partial mutation. Combat configuration is validated, runtime combat profiles use real actor levels, and hit/reward/growth/inventory/hospital boundary arithmetic is checked, saturated, or rejected deliberately. Verification passed `102/102` focused tests and `999/999` full-suite tests; the framework retained `0` warnings, the solution retained `98` legacy-host warnings, and all clean demos passed. Capability statuses remain unchanged because this is framework correctness work, not protected consumer retirement.

Review-Whole-7 closes the direct-definition immutability gap and aligns affinity decisions. Custom parameters now accept only a closed serializer-neutral CLR value algebra and recursively copy, normalize, depth-check, and cycle-check nested values. Fusion result parameters use that same boundary. Runtime affinity conditions and damage now share passive-aware resolution, including deterministic protection from self-referential affinity modifiers and owner-correct modifier conditions. Clean AI evaluates every target and damage element for multi-target actions; random actions account for every target execution could choose. Verification passed `113/113` focused tests and `1007/1007` full-suite tests; the framework retained `0` warnings, the solution retained `98` protected legacy-host warnings, all four clean demos passed, and `Data/Jsons` remained unchanged. Capability statuses and removal authorization remain unchanged.

Review-Whole-8 completes the medium-severity neutrality corrections from the framework-wide review. The floor/menu-oriented dungeon compatibility service is physically host-owned; framework wallets/rewards use neutral currency terms; negotiation requires explicit host or game policy; and stock-capacity tiers are authored or injected rather than silently inherited from the prototype. The legacy console behavior remains available through host policies and adapters. Verification passed `260/260` focused tests and `1013/1013` full-suite tests; the framework retained `0` warnings, the solution retained `98` protected legacy-host warnings, and all four clean demos passed. This is an authority-boundary correction, not a capability-parity promotion, so removal authorization remains unchanged.

The L3 hardening pass closes catalog actor-creation level and initialization integrity. Optional progression must match the requested level, that one level controls unlocks and initialization, and malformed policy resources return typed diagnostics without constructing a partial actor or throwing through the factory boundary. Verification passed `24/24` focused actor tests, `189/189` dependent integration tests, and `1017/1017` full-suite tests. Capability status and removal authorization remain unchanged.

The L4 hardening pass closes completed-encounter snapshot integrity. Live participant state exists only while the encounter is executing; every returned participant is a detached full actor snapshot captured after battle-end lifecycle work, including fault-before-start results. The automated battle facade consumes that frozen result rather than rereading live actors. Verification passed `25/25` focused encounter tests, `169/169` dependent integration tests, and `1020/1020` full-suite tests. Capability status and removal authorization remain unchanged.

The L5 hardening pass closes encounter synchronization-context deadlock risk. The framework async runner uses context-free awaits throughout, while the compatibility-only synchronous helper temporarily detaches and then restores any caller context. Godot and UI hosts still await `RunAsync` and own all engine-thread marshaling. Verification passed `27/27` focused encounter tests, `171/171` dependent integration tests, and `1022/1022` full-suite tests. Capability status and removal authorization remain unchanged.
...
| Negotiation clean host flow | Training Annex uses authored demands and an explicit clean session policy; broader content and consumer parity remain unfinished | TBD |
| Fusion clean host flow | Phases 7-30 through 7-35 prove result, planning, preview, sample transaction, explicit strategies, and Compendium paths; CodeReview-7-1 through CodeReview-7-5 materially improve those paths, and all four medium re-review findings are corrected | Phase 7 is review-closed; proceed to Phase 8-36 host interchangeability |
...
Phase 5-28 verification passed `76/76` focused Training Annex host tests and `847/847` full-suite tests with no skips. Phase 6-29 is implemented: the Training Annex clean host demonstrates negotiation/recruitment through framework session, recruitment, party-stock, and economy services, including success, refusal, and repeated familiar paths. Review-Whole-8 subsequently made authored clean demands authoritative at the host content boundary and made all session defaults explicit through injected policy. Phase 6-29 verification passed `104/104` focused Training Annex, party-stock, parity-ledger, and original-content tests and `851/851` full-suite tests with no skips. The framework build has `0` warnings, the complete solution retains `98` pre-existing legacy-host warnings, all four noninteractive clean demos pass, `git diff --check` passes, and the framework forbidden-reference search returns no matches. `Data/Jsons` changed only for the clean Training Annex negotiation sample.

Phase 7-30 is implemented: clean fusion result calculation now uses `CatalogFusionContentRepository` over the original Training Annex pack and records non-mutating result evidence in `--clean-training-annex-play`. Phase 7-31 is implemented: the same command now records clean planning evidence for inheritance slots, passive/explicit-allowance filtering, already-known and blocked reason codes, and a deterministic mutation/accident sample from `Echo Strike` to `Shell Bash`. Phase 7-32 is implemented: `Preview Fusion Result` validates inherited-skill selection through the framework planning service and creates a Ward Shell preview through `FusionPreviewService`. Phase 7-33 is implemented: `Commit Fusion Transaction` rejects duplicate results before mutation and can atomically consume parent demons/add a fused Ward Shell through framework party-stock transitions. Phase 7-34 is implemented: fusion resolution/planning require explicit strategy policies, clean requests no longer carry mandatory Moon Phase data, typed catalyst boosts no longer inspect names, and legacy assumptions live only in the console compatibility policy. Phase 7-35 is implemented: clean Compendium registration, recall, stock/wallet transaction ownership, persistence, and opt-in familiar-player-knowledge import now run through framework contracts. CodeReview-7-1 through CodeReview-7-5 improved recipe fidelity, identity checks, transaction ownership, policy context, and Compendium integrity. The fresh 2026-07-11 source audit found four additional medium gaps, all corrected by 2026-07-12: neutral structured results use catalog state, previews require planner-issued validated selections, restored Demon/Persona stock shares live capacity policy, and duplicate knowledge keys are rejected with typed diagnostics before restore or familiar import. Both lower-priority findings are also corrected: direct party additions reject incompatible form/stock IDs while preserving intentional overlaps, and Compendium recall pricing is an optional host policy with no framework-owned currency name. Phase 7 is review-closed; Phase 8-36 may proceed under the remaining parity safeguards.
...
CodeReview-7-2 verification passed `166/166` focused identity, recall, stock, persistence, host, and ledger tests plus `915/915` full-suite tests. The framework build remained at `0` warnings, the solution retained `98` protected legacy-host warnings, all four clean demos passed, boundary and diff checks passed, and `Data/Jsons` remained unchanged.
