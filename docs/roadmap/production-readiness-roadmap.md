...
This document is the active completion record for the findings first recorded
...
| PR-B3 | Public and authoring contracts are uncontrolled | `verified` | The 0.1 API, schema-v3 contracts, and complete authoring validator CLI are guarded and accepted by the consolidated review. |
| PR-B4 | Godot compatibility is only simulated | `verified` | The real Godot 4.7.1 .NET sample executes content, commands, actions, encounters, Nodes, events, and host-owned saves. |
| PR-B5 | No complete repository release gate | `verified` | Locked audited restore, strict build and formatting, API/schema/content checks, coverage thresholds, all DemoHost modes, real Godot smoke, trimming analysis, documentation links, and security policy form one repeatable gate. |
...
| PR-M4 | Authored rulesets are mostly fixed labels | `verified` | Hosts supply a typed factory registry; standard damage exposes all combat settings; roster tiers and Action Token liveness are authored; fixed supplied policies remain replaceable. |
| PR-M5 | Encounter events leak presentation and omit data | `verified` | Every event kind carries an immutable typed payload; optional debug text is never the sole data source; exact signed resource changes were corrected in `4c07ac4`. |
| PR-M6 | Aggregate save restoration is host-reimplemented | `verified` | Framework owns validated, dependency-ordered, all-or-nothing session restoration and an explicit migration-step seam. |
| PR-L1 | Runtime random targeting has an ordered fallback | `verified` | Both targeting policies are mandatory; ordered behavior requires explicit injection. |
| PR-L2 | DemoHost flattens content filenames | `verified` | Build output preserves pack directories; documents resolve relative to their manifest under the confined content root. |
| CR-M6 | Catalyst rank shifting implements the wrong rule | `verified` | Schema v3 uses authored catalyst/target roles, exact same-race rank lookup, and typed rejection without clamping. |

The later code review added `CR-M6`: catalyst rank shifting implemented the
wrong rule. It was implemented and accepted under the same source-based review
standard as the original audit findings.
...
Every checkpoint received focused tests, the full solution gate, strict
nonincremental builds, formatting verification, applicable demos, documentation
checks, and its own green commit. The consolidated review then promoted findings
from `implemented_pending_review` to `verified` only after its demonstrated
resource-event defect was corrected.
...
| 4 | `084fd2f` (`battle: structure encounter event contracts`) | 800 tests passed; 0 skipped; all event kinds are payload-checked, structural collections are immutable, turn-economy changes include before/after state and consumption, and final outcomes/faults are typed. Exact resource mutation reporting was corrected in `4c07ac4`. |
| 5 | `80f696f` (`runtime: add aggregate session restoration`) | 805 tests passed; 0 skipped; current-version restore, missing migration paths, explicit migration steps, Hosted Entity ordering, missing dependencies, dependency cycles, immutable aggregate views, and actor failure atomicity covered. |
| 6 | `9b7ac5a` (`host: preserve content pack paths`) | 807 tests passed; 0 skipped; all active content retains its pack-relative output path, document lookup is manifest-relative and root-confined, and identical document basenames in separate packs remain isolated. |
| 7 | `2751c82` (`api: establish convergence 0.1 contract`) | Assembly/API version `0.1.0`; 9,531 shipped API signatures guarded by PublicApiAnalyzers 5.6.0; XML documentation emitted for supported composition entry points; implementation namespaces export no public types. |
| 8 | `aee426a` (`schema: publish content schema v3`) | 14 Draft 2020-12 schema artifacts cover 13 document families plus shared definitions; all 36 active documents declare the matching URN and pass independent JsonSchema.Net 9.2.2 evaluation; 106 focused contract cases cover valid unions and structural rejection; 914 full-suite tests passed with 0 skipped and 0 build warnings. |
...
| 10 | `578f24b` (`host: add godot 4.7 reference consumer`) | `Godot.NET.Sdk` 4.7.1 builds with zero warnings; the official 4.7.1 .NET engine headlessly loads one `res://` pack, maps two runtime actors to Nodes, executes a typed skill, consumes 18 ordered encounter events, restores a two-actor host-owned save, and exits 0; 925 full-suite tests passed with 0 skipped and 0 build warnings. |
| 11 | `baa7f56` (`quality: complete production release gate`) | Locked vulnerability-audited restore passed for all seven active projects; 927 full-suite tests passed with 0 skipped; strict Release and trim-aware builds produced 0 warnings; Framework coverage measured 91.15% lines and 75.22% branches; 6 packs, 36 documents, and 94 definitions passed schema and catalog validation; all four noninteractive demos, scripted Training Annex input, and the real Godot 4.7.1 headless smoke exited successfully. |
| Review correction | `4c07ac4` (`fix: report exact encounter resource changes`) | Exact signed costs and effect resource changes now drive encounter events; non-resource effects no longer masquerade as resource mutations; focused and full regressions pass. |
...
Every ledger entry is now `verified`, the post-correction release gate is green,
and the final source-based review found no unresolved release blocker. The
roadmap remains an active release record rather than being archived merely
because its implementation commits exist.

Final post-review evidence: 930 tests passed with 0 failed and 0 skipped; strict
Release and trim-aware builds produced 0 warnings; Framework coverage measured
91.17% lines and 75.22% branches; all active content, DemoHost modes, scripted
Training Annex play, and the official Godot 4.7.1 .NET headless smoke passed.
