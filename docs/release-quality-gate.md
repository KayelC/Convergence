...
`Convergence.ContentValidator` validates every active JSON document against schema v7 and the Framework's deserialization, semantic, dependency, registration, and catalog rules. The gate then runs all four noninteractive DemoHost modes and a scripted `--clean-training-annex-play` session.
